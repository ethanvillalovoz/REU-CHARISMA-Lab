...
# Program: geometryScript3.0
# Description: Calculates the final position for X amount of robots around a reference point and orientation
...
def rotate_around_point(x,y, ox,oy, degrees):
...
def square(shape):
    ...
    squareInfo = userInformation(shape)
    ...
    for j in range(0, numPoints, 2):
        ...
        qx, qy = rotate_around_point(x, y, ox, oy, squareInfo[4])
...
def triangle(shape):
    ...
    # Retrieves the info required to compute
    triangleInfo = userInformation(shape)

    if triangleInfo[3] % 2 == 0:
        # We have an even number of robots

        robotNum = math.ceil(float(triangleInfo[3]) / 2) + 1
        ...
        for j in range(0, numPoints, 2):
            ...
            qx, qy = rotate_around_point(x, y, ox, oy, -45)
        ...
        triangleGoal = np.concatenate((triangleGoal, line2_trim))
    ...
    for j in range(0, numPoints, 2):
        ...
        qx, qy = rotate_around_point(x, y, ox, oy, triangleInfo[4])
...
def semiCircle(shape):

    # Retrieves the info required to compute
    semiCirlceInfo = userInformation(shape)
    ...
    if semiCirlceInfo[4] == 0:
        ...
        for j in range(0, numPoints, 2):
            ...
            qx, qy = rotate_around_point(x, y, ox, oy, semiCirlceInfo[4])
...
def clump(shape):
    ...
    # Retrieves the info required to compute
    clumpInfo = userInformation(shape)

    clumpGoal = np.zeros(int(clumpInfo[3]) * 2)

    feature = input("How would you like the configuration of the clump(angular/rounded: ").lower()
    ...
    for j in range(0, numPoints, 2):
        ...
        qx, qy = rotate_around_point(x, y, ox, oy, clumpInfo[4])
...
def circle(shape):
    ...
    # Retrieves the info required to compute
    circleInfo = userInformation(shape)
    ...
    for j in range(0, numPoints, 2):
        ...
        qx, qy = rotate_around_point(x, y, ox, oy, circleInfo[4])
...
def line(shape):
    ...
    # Retrieves the info required to compute
    lineInfo = userInformation(shape)
    ...
    # Custom Orientation
        ...
        for j in range(0, numPoints, 2):
            ...
            qx, qy = rotate_around_point(x, y, ox, oy, lineInfo[4])
...
def main():
    ...
    ## Future implementation pushing data to csv file for global planner
